...
(function () {
  ...
  var STONES = {
    ...
    /* The hero slab: dark polished stone, wide veining, seen up close. */
    hero: function (ctx, w, h, rand) {
      ctx.fillStyle = '#0d1114';
      ctx.fillRect(0, 0, w, h);
      bands(ctx, w, h, rand, ['#141a1e', '#0f1417', '#182025']);
      speckle(ctx, w, h, rand, {
        density: 0.012, min: 0.5, max: 2.4, alpha: [0.06, 0.22],
        colors: ['#59636a', '#8d979d', '#2a3238']
      });
      for (var i = 0; i < 4; i++) {
        vein(ctx, w, h, rand, {
          start: 0.12 + i * 0.24, tilt: 0.05 + rand() * 0.1, wander: 0.3,
          width: 1.4 + rand() * 2.4,
          color: i === 1 ? '#c9a227' : '#7d8a91',
          halo: i === 1 ? '#6d5a1c' : '#39434a',
          alpha: i === 1 ? 0.34 : 0.3
        });
      }
    }
